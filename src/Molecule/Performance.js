...
function Performance(props) {
    ...
    const onPerformance = () => {
        axios.get("http://${NetworkConfig.networkAddress}:8080/workList/prepare",  {withCredentials: true})
            .then((res) => {
                setPRows(res.data.data)
            });
    }
    const onOrganList = () => {
        axios.get("http://${NetworkConfig.networkAddress}:8080/file/registrationStatus", {withCredentials: true})
            .then((res) => {
                setOgRows(res.data.data)
            })
    }
    const onOverall = () => {
        axios.get("http://${NetworkConfig.networkAddress}:8080/workList/overall", {withCredentials: true})
            .then((res) => {
                setORows(res.data)
            })
    }
